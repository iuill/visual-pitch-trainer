...
「ピッチ推定」では、音源ファイル解析の推定器を選べます。通常はおすすめの `RMVPE`、軽い環境では `pitchy`、比較したい場合は `CREPE small / medium / large / full` も選べます。マイクのリアルタイム音程検出は引き続き軽量な `pitchy` を使います。CREPE は公開成果物に含めた `yqzhishen/onnxcrepe` のモデル、RMVPE は公開成果物に含めた `lj1995/VoiceConversionWebUI` の `rmvpe.onnx` を使います。別のモデルを使う場合は、ビルド時に `VITE_CREPE_MODEL_URL` や `VITE_RMVPE_MODEL_URL` へ ONNX モデルの URL を指定してください。

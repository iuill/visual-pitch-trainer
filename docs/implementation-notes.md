...
ただし、手書き実装の調整には限界があったため、既存ライブラリの利用に切り替えた。`pitchfinder` は YIN と McLeod Pitch Method を併用できたが、ライセンス面の懸念があるため、現在は MIT ライセンスで配布されている `pitchy` の McLeod Pitch Method を使っている。`pitchy` はブラウザで `Float32Array` を直接扱えるため、Web Audio API のマイク入力と相性がよい。
...
1. `pitchy` の McLeod Pitch Method を実測する
2. 安定しなければ WASM 系を検討する

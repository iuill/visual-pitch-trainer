...
ボーカル抽出は WebGPU 対応ブラウザを優先する。スマートフォンや WebGPU 非対応ブラウザでは利用できない場合がある。初回はモデル取得の外部通信が発生するが、音声データや解析結果は外部へ送信しない。
...
ライセンス面では、Mediabunny は MPL-2.0、demucs-web と ONNX Runtime Web は MIT として扱う。ONNX Runtime Web の依存に Apache-2.0、BSD-3-Clause、ISC のパッケージが含まれるため、`public/THIRD_PARTY_NOTICES.txt` に追記している。ボーカル抽出モデルはアプリに同梱せず、demucs-web が既定で参照する Hugging Face 上の `timcsy/demucs-web-onnx` の `htdemucs_embedded.onnx` を実行時に取得する。

...
- 開発用にピッチ推定モデル比較PNGと全ボーカル抽出モデルWAVを出力
...
開発用ツールとして、同じ解析用音源を全ボーカル抽出モデルで順番に処理し、抽出ボーカルをモデルごとの WAV として出力できる。比較出力は `src/app.ts` の `handleVocalSeparationComparisonCapture()` が担当し、各モデルの失敗はログに残して次のモデルを続行する。ローカルONNXを生成していないBS-RoFormerモデルは取得エラーとして失敗扱いになる。

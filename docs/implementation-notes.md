...
- 練習サマリーの表示
- ボーカルのみの音源ファイルをブラウザ内で読み込み、歌声らしいピッチから声域を推定表示
...
音源ファイル解析は `src/audioFileAnalysis.ts` に分離している。

現在の処理:

- `input[type="file"]` で選択された音源を `AudioContext.decodeAudioData()` で読み込む
...
アプリ内では AI によるボーカル分離は行っていない。伴奏、ベース、リード楽器などが強い音源では歌声以外の音程を拾うため、伴奏入り音源を直接解析対象にしない。通常の楽曲を使う場合は、先に `audio-separator` などの外部ツールでボーカルのみの音源を作り、その出力をアプリへ読み込ませる。

GPU が使える Docker 環境では、例えば以下のコマンドでボーカル抽出を行う。

...
ライセンス面では、Mediabunny は MPL-2.0、demucs-web と ONNX Runtime Web は MIT として扱う。ONNX Runtime Web の依存に Apache-2.0、BSD-3-Clause、ISC のパッケージが含まれるため、`public/THIRD_PARTY_NOTICES.txt` に追記している。Workers Static Assets の単一ファイルサイズ上限を避けるため、ONNX Runtime Web の WASM ファイルはアプリに同梱せず jsDelivr から実行時に取得する。ボーカル抽出モデルもアプリに同梱せず、demucs-web が既定で参照する Hugging Face 上の `timcsy/demucs-web-onnx` の `htdemucs_embedded.onnx` を実行時に取得する。
